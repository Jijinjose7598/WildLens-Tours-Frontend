...
const Booking = () => {
  ...
  useEffect(() => {
    const fetchHotelsAndTravels = async () => {
      try {
        const hotelsResponse = await axios.get('https://wildlens-tours-backend-tqh1.onrender.com/api/hotels/');
        setHotels(hotelsResponse.data);

        // Fetch travels data if needed
        // const travelsResponse = await axios.get('https://wildlens-tours-backend-tqh1.onrender.com/api/travels/');
 ...
 useEffect(() => {
  ...
  const handleBooking = async (event) => {
    ...
    try {
      const bookingResponse = await axios.post('https://wildlens-tours-backend-tqh1.onrender.com/api/bookings/tour/create', {
        name,
        startDate,
        endDate,
        adults,
