...
const Packages = () => {
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      ...
      setError('');
      try {
        const response = await axios.get('https://wildlens-tours-backend-tqh1.onrender.com/api/packages/');
  ...
  useEffect(() => {
    if (pathname.startsWith('/packages/') && packageId) {
      const fetchPackageDetails = async () => {
        ...
        setError('');
        try {
          const response = await axios.get(`https://wildlens-tours-backend-tqh1.onrender.com/api/packages/${packageId}`);
  ...
  const handleDeletePackage = async (pkgId) => {
    if (window.confirm('Are you sure you want to delete this package?')) {
      try {
        await axios.delete(`https://wildlens-tours-backend-tqh1.onrender.com/api/packages/delete/${pkgId}`);
